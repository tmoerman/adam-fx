package org.tmoerman.adam.fx.rdd
...
/**
 * --[NOTE]-----------------------------------------------------------------------
 * Copied from org.bdgenomics.adam.rdd.BroadCastRegionJoin.
 * The ADAM version doesn't exactly inspire reuse of the algorithmic constituents.
 * -------------------------------------------------------------------------------
 *
 ...
 */
object CustomBroadcastRegionJoin extends RegionJoin {
...
//  def apply(values: Seq[ReferenceRegion]) = {
//    new MultiContigNonoverlappingRegions(
//      values
//        .map(v => (v.referenceName, v))
//        .groupBy(t => t._1)
